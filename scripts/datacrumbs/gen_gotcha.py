...
Emits one X-macro entry per symbol from headers parsed with libclang. A generic wrapper cannot
forward arguments without the real prototype, and 381 symbols do not scale by hand. Variadic
functions are skipped and reported, since forwarding a va_list needs a v-form of the callee.
"""
...
# nor the field type names the enum. Only the association is written here. The offset and width
# come from the headers, so a struct that gains a member does not shift what gets read.
#
...
def prototypes(header, includes, all_enums, enum_src=None):
    ...
    for c in tu.cursor.get_children():
        if c.kind == cix.CursorKind.ENUM_DECL:
            ...
            # ones worth decoding. Key those by the shared prefix instead. An anonymous enum is
            # spelled "enum (unnamed at file:line)", not empty.
