...
// guard is false and the version records vanish without a diagnostic. One list per library, not
// one holding all of them, so a build without a library never carries its surface.
...
namespace {
// cat groups by library surface, type names the instrumentation that produced the record.
...
// The generated wrappers put each symbol's own operation class in cat instead; DC_CAT_VARIADIC
// is only for the trampolined symbols that no header declares.
...
/// Records the libraries this build decoded against, and warns when the running one differs.
///
/// Opcode and flag names are compiled in from the headers, so a mismatch decodes to a plausible
/// wrong name rather than failing. Only DOCA exposes a runtime version to check against.
void check_version() {
...
/// `include` defaults to everything and runs first; `exclude` runs after and wins.
/// `classes` matches the role recorded in the wrap list, so a selection can say
/// `submit` instead of naming every symbol that submits. Patterns are fnmatch globs.
...
void datacrumbs::client::wrap::init() {
  // Idempotent: both the standalone library and a client build that compiles this surface in call
  // init. Wrapping twice through GOTCHA would chain the wrapper onto itself.
  ...
  // a workload that called nothing, so the count is reported rather than inferred from a trace.
...
///
/// @p sp is the stack pointer at entry, which identifies the frame on the way back: a longjmp or
/// an unwinding exception abandons frames without returning through them.
struct ShadowFrame {
  const char* name;
  void* ret;
  std::uintptr_t sp;
...
/// The capture mode of a trampolined symbol, resolved once per symbol and cached. The names
/// are the binding table's own strings, so the pointer is the key. Recompiling the selection
/// regex on every call is too costly for a hot poll loop.
datacrumbs::ConfigurationManager::CaptureMode tramp_mode(const char* name) {
  static std::unordered_map<const char*, datacrumbs::ConfigurationManager::CaptureMode> modes;
  static std::shared_mutex lock;
  {
    std::shared_lock<std::shared_mutex> r(lock);
    const auto it = modes.find(name);
    if (it != modes.end()) return it->second;
  }
  const auto mode = datacrumbs::client::gotcha_wrap::mode_of(name);
  std::unique_lock<std::shared_mutex> w(lock);
  modes.emplace(name, mode);
  return mode;
}

void emit_tramp(const char* name, std::uint64_t t0, std::uint64_t dur_ns) {
  // Not cached: sink() only rechecks an init guard, and this already runs once per call.
  datacrumbs::client::PfwSink* s = sink();
  if (s == nullptr) return;
  if (tramp_mode(name) == datacrumbs::ConfigurationManager::CaptureMode::AGGREGATE) {
  ...
  const int n = std::snprintf(
      ...
      "\n",
      static_cast<unsigned long long>(s->next_id()), name,
      static_cast<long>(datacrumbs::client::self_pid()),
...
/// @p lr_out receives the link register to install: the return thunk when the call was
/// recorded, or the caller's own return address when it was not, so an unrecordable
/// call still returns correctly.
extern "C" void* dc_tramp_enter(const char* name, void** slot, void* ret, void* sp, void** lr_out) {
  ...
  if (target == nullptr) {
    ...
    // A target inside the shim is the trampoline itself: seen when DOCA is dlopened after it.
    Dl_info here, there;
    if (dladdr(reinterpret_cast<void*>(&dc_tramp_enter), &here) != 0 &&
        dladdr(target, &there) != 0 && here.dli_fbase == there.dli_fbase) {
      DC_LOG_ERROR("[api] %s resolves back into the shim (%s); load the library that defines it "
                   "before the shim",
                   name, here.dli_fname != nullptr ? here.dli_fname : "?");
      std::abort();
    }
  ...
  // A tracer thread's own vendor calls are forwarded unrecorded, same as the wrapped path's Guard.
  if (in_tramp != 0 || sink() == nullptr || datacrumbs::client::tracer_thread_active())
    return target;
  // A symbol the selection turns off opens no frame: the jump is its whole cost.
  if (tramp_mode(name) == datacrumbs::ConfigurationManager::CaptureMode::OFF) return target;
...
/// Pops until the recorded stack pointer matches, because frames abandoned by a
/// longjmp or unwinding exception never return through here and would otherwise
/// sit on the stack forever, shifting every later return onto the wrong address.
...
void datacrumbs::client::wrap::fini() {
  // Destroy the sink without unbinding: the destructor drains the rings and tallies, then flushes.
  // A wrapper called after this sees a null sink and records nothing. Unbinding first can
  // truncate the trace output.
  datacrumbs::Singleton<ApiSink>::finalize();
}
...
// does not know it exists, so nothing else calls init here. The configuration and the sink are
// both built on first use, so calling this early is safe.
