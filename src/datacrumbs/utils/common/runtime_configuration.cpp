...
/// Set, non-empty and not "0". Anything else counts as on, so "1", "yes", and "all" all mean on.
...
/// The environment, else what the configurator resolved from the probe yaml. A member, so it
/// lives exactly as long as the configuration it fills: a file-scope cache would be destroyed
/// before this object during shutdown, leaving a reader holding a dangling map.
...
/// Unset means on, unlike setting_flag. These settings bound the trace, so a run that does not
/// set them must still be bounded.
...
bool matches_any(const std::string& patterns, const char* name) {
  ...
  while (start <= patterns.size()) {
    ...
    if (!one.empty()) {
      ...
      if (is_regex) {
        try {
          // ECMAScript, not POSIX ERE: the probe yaml is written with lookahead, which ERE
          // cannot parse, and a pattern that fails to compile selects nothing at all.
          if (std::regex_match(name, std::regex(one))) return true;
...
void ConfigurationManager::load_runtime_system_configuration() {
  ...
  engine_join_enabled = setting_flag_default_on(DATACRUMBS_ENV_ENGINE_JOIN);
  engine_scan_cache_enabled = setting_flag_default_on(DATACRUMBS_ENV_ENGINE_SCAN_CACHE);
  const std::string drain_us = setting(DATACRUMBS_ENV_ENGINE_DRAIN_US);
  if (!drain_us.empty()) {
    const int v = std::atoi(drain_us.c_str());
    if (v > 0) engine_drain_us = static_cast<unsigned int>(v);
  }
  engine_capture_mode = setting(DATACRUMBS_ENV_ENGINE_CAPTURE_MODE);
  const std::string engine_sample_str = setting(DATACRUMBS_ENV_ENGINE_SAMPLE);
  if (!engine_sample_str.empty()) {
    const int v = std::atoi(engine_sample_str.c_str());
    if (v > 0) engine_sample = static_cast<unsigned int>(v);
  }
  ...
  const std::string anchor_interval = setting(DATACRUMBS_ENV_DPA_ANCHOR_INTERVAL_S);
  if (!anchor_interval.empty()) {
    const int v = std::atoi(anchor_interval.c_str());
    if (v > 0) dpa_anchor_interval_s = static_cast<unsigned int>(v);
  }
  dpa_dev = setting(DATACRUMBS_ENV_DPA_DEV);
  dpa_anchor_mode = setting(DATACRUMBS_ENV_DPA_ANCHOR_MODE);
  const std::string burst_us = setting(DATACRUMBS_ENV_DPA_ANCHOR_BURST_US);
  if (!burst_us.empty()) {
    const int v = std::atoi(burst_us.c_str());
    if (v > 0) dpa_anchor_burst_us = static_cast<unsigned int>(v);
  }
  ...
  const std::string ring = setting(DATACRUMBS_ENV_RING_MB);
  if (!ring.empty() && std::atoi(ring.c_str()) > 0)
    ring_mb = static_cast<unsigned>(std::atoi(ring.c_str()));
  const std::string cthreads = setting(DATACRUMBS_ENV_COMPRESS_THREADS);
  if (!cthreads.empty() && std::atoi(cthreads.c_str()) >= 0)
    compress_threads = static_cast<unsigned>(std::atoi(cthreads.c_str()));
  worker_cpus = setting(DATACRUMBS_ENV_WORKER_CPUS);
  ...
  dpa_keep_anchor = setting_flag(DATACRUMBS_ENV_DPA_KEEP_ANCHOR);
  const std::string rearm = setting(DATACRUMBS_ENV_DPA_REARM);
  if (rearm == "all" || rearm == "process" || rearm == "off") dpa_rearm = rearm;
  ...
  const std::string ci_interval_ms = setting(DATACRUMBS_ENV_TIMESYNC_CLOCK_INFO_INTERVAL_MS);
  if (!ci_interval_ms.empty())
    timesync_clock_info_interval_ms = static_cast<unsigned int>(std::atoi(ci_interval_ms.c_str()));
  if (timesync_clock_info_interval_ms == 0) timesync_clock_info_interval_ms = 60000u;
  ...
  posix_select = {setting(DATACRUMBS_ENV_POSIX_RECORD), setting(DATACRUMBS_ENV_POSIX_AGGREGATE),
                  setting(DATACRUMBS_ENV_POSIX_OFF)};
  stdio_mode = mode_for_module(stdio_enabled);
  stdio_select = {setting(DATACRUMBS_ENV_STDIO_RECORD), setting(DATACRUMBS_ENV_STDIO_AGGREGATE),
                  setting(DATACRUMBS_ENV_STDIO_OFF)};
  api_mode = mode_for_module(api_enabled);
  api_select = {setting(DATACRUMBS_ENV_API_RECORD), setting(DATACRUMBS_ENV_API_AGGREGATE),
                setting(DATACRUMBS_ENV_API_OFF)};
  engine_mode = mode_for_module(engine_enabled);
  engine_select = {setting(DATACRUMBS_ENV_ENGINE_RECORD), setting(DATACRUMBS_ENV_ENGINE_AGGREGATE),
                   setting(DATACRUMBS_ENV_ENGINE_OFF)};
  hwts_mode = mode_for_module(hwts_enabled);
  hwts_select = {setting(DATACRUMBS_ENV_HWTS_RECORD), setting(DATACRUMBS_ENV_HWTS_AGGREGATE),
                 setting(DATACRUMBS_ENV_HWTS_OFF)};

  // These per-module mode variables no longer have any effect; the two globals above replace them.
  // If one is still set, warn instead of silently running the module in the wrong mode, which
  // would look like the workload itself had changed.
  for (const char* gone : {"DATACRUMBS_POSIX_MODE", "DATACRUMBS_STDIO_MODE", "DATACRUMBS_API_MODE",
                           "DATACRUMBS_ENGINE_MODE", "DATACRUMBS_HWTS_MODE",
                           "DATACRUMBS_ENGINE_RECORD", "DATACRUMBS_ENGINE_AGGREGATE"}) {
