...
// Enumerates kernel tracepoints from tracefs.
// Each tracepoint name has the form "category:name". Each event has a directory
// under events/<category>/ that contains a "format" file.
class TracepointCapture {
 public:
      ...
      for (const auto& cat : fs::directory_iterator(root, ec)) {
        ...
        for (const auto& ev : fs::directory_iterator(cat.path(), ec)) {
          ...
          if (fs::exists(ev.path() / "format")) {
            tracepoints_.insert(cat.path().filename().string() + ":" +
  ...
  std::vector<std::string> getFunctionsByRegex(const std::string& pattern) const {
    ...
    // tracefs requires root access, so enumeration can come back empty for a normal user.
    // In that case, fall back to literal "category:name" tokens found in the pattern
    // itself, so an explicit list still works without tracefs access.
  ...
  std::unordered_set<std::string> tracepoints_;
};
