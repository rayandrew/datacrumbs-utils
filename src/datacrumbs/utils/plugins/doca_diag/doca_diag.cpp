// Node-scope plugin: the API needs root and the counter list applies device-wide,
// so per-process scope would race between traced processes.
// These are NIC-internal counters, not the host's view. The completion engine
// count is exact: 1001 submitted DMA ops read back as 1001.
...
#define DC_GROUP(g, n, arr) {(g), (n), (arr), sizeof(arr) / sizeof((arr)[0])}
...
/// Reads the buffered samples for one tick and emits one record per group. Sized and read in
/// entries, not bytes: asking for a count in entries while sizing in entries overruns the
/// buffer by the number of ids, which is a crash, not a short read.
void sample(const datacrumbs::PluginEmit& emit) {
  ...
  if (!g_state.started) {
    ...
    const doca_error_t sz = doca_telemetry_diag_get_sample_size(g_state.diag, &g_state.sample_size);
...
extern "C" bool datacrumbs_plugin_register(const datacrumbs::PluginApi* api) {
  ...
  // The counters are per function. The traced device is the first of DATACRUMBS_NIC_DEVICES;
  // any other function that supports them counts someone else's traffic.
  std::string want;
  if (const char* devs = std::getenv("DATACRUMBS_NIC_DEVICES"); devs != nullptr) {
    want = devs;
    want = want.substr(0, want.find(','));
  }
  ...
  std::string opened;
  for (int pass = want.empty() ? 1 : 0; pass < 2 && g_state.dev == nullptr; pass++)
    for (uint32_t i = 0; i < n && g_state.dev == nullptr; i++) {
      if (doca_telemetry_diag_cap_is_supported(list[i]) != DOCA_SUCCESS) continue;
      if (doca_devinfo_get_ibdev_name(list[i], name, sizeof(name)) != DOCA_SUCCESS) name[0] = 0;
      if (pass == 0 && want != name) continue;
      if (doca_dev_open(list[i], &g_state.dev) == DOCA_SUCCESS) opened = name;
    }
  ...
  if (!want.empty() && opened != want)
    DC_LOG_WARN("doca diag: %s has no diagnostic counters, sampling %s instead", want.c_str(),
                opened.c_str());
  else
    DC_LOG_INFO("doca diag: sampling %s", opened.c_str());
  ...
  for (unsigned g = 0; g < kGroupCount; g++) {
    ...
    for (unsigned k = 0; k < kGroups[g].count; k++) {
      const uint64_t id =
          (static_cast<uint64_t>(kGroups[g].group) << kIdGroupShift) |
  ...
  doca_telemetry_diag_set_sample_period(g_state.diag, static_cast<uint64_t>(interval) * 1000000ULL);
