// dc_timesync: cross-node time reconciliation between two NIC PTP hardware clocks (PHCs).
// A two-step HW-timestamped UDP exchange over IPv6 link-local on mlx5, plus the local
// PHC<->CLOCK_MONOTONIC bridge, remaps events onto one cross-node timeline without steering
// the host clock. Roles: resp/init run the primitive and a self-test; daemon adds a sliding fit.

// TX hw timestamps come from the socket error queue (MSG_ERRQUEUE). RX ones arrive as
// SCM_TIMESTAMPING cmsgs. Needs CAP_NET_ADMIN (sudo) for SIOCSHWTSTAMP.
...
// Drain any stale packets (normal + error queue) so a round uses only fresh timestamps.
// Prevents a slow-turnaround peer from building a backlog that drifts RX timestamps later each
// round.
...
// Fully drains the queue and returns the freshest (last) timestamp. If more TX timestamps
// accumulated than were drained, for example a two-packet turnaround, the oldest would
// otherwise be returned and drift staler every round.
...
// Populate the local clock registry: for each /dev/ptpN, the offset to the SYNCED PHC's ns (the
// one the cross-node fit uses). BlueField has several independent PHCs and the fit covers only
// one, so without this a timestamp from another PHC lands far off. Both reads use
// PTP_SYS_OFFSET_PRECISE; the PHCs share the NIC oscillator, so relative drift between them stays
// small.
...
// The DOCA fabric CQE hw timestamp is the HCA free-running clock, not exposed via /dev/ptp. When
// DC_TIMESYNC_RAW_DEV names the fabric ib device, this reads that clock via ibv_query_rt_values_ex
// each cycle and publishes a raw->synced-PHC fit, so the DOCA CQE hook can map a completion
// timestamp onto the global epoch with no local anchor.
...
// Cache of the clock_info snapshot handed to readers, refreshed every
// DC_TIMESYNC_CLOCK_INFO_REFRESH_MS rather than every cycle: mlx5dv_get_clock_info's page is not
// guaranteed to advance within a second, so refreshing more often can give the fit a wrong skew.
// Conversion goes through datacrumbs_timesync_raw_ts_to_ns since the vendor's own mlx5dv_ts_to_ns
// overflows in uint64_t.
static struct dc_timesync_snapshot g_raw_ci_pub;
static int g_raw_ci_pub_valid = 0;
static uint64_t g_raw_ci_pub_mono_ns = 0;
static int g_raw_ci_refresh_ms = 60000;

// Default 60s (DC_TIMESYNC_CLOCK_INFO_REFRESH_MS): clock_info.mask is 41 bits on known hardware, so
// the raw-to-ns conversion is correct only while a raw timestamp's gap to last_cycles stays under
// mask/2, about 18.3 minutes at 1 cycle/ns. Refreshing every 60s keeps each conversion a small
// fraction of that bound.
static void maybe_refresh_clock_info(uint64_t now) {
  if (!g_raw_ctx) return;
  if (g_raw_ci_pub_valid && now - g_raw_ci_pub_mono_ns < (uint64_t)g_raw_ci_refresh_ms * 1000000ull)
    return;
  struct mlx5dv_clock_info ci;
  if (mlx5dv_get_clock_info(g_raw_ctx, &ci) != 0) return;
  g_raw_ci_pub.raw_clock_info_nsec = ci.nsec;
  g_raw_ci_pub.raw_clock_info_last_cycles = ci.last_cycles;
  g_raw_ci_pub.raw_clock_info_frac = ci.frac;
  g_raw_ci_pub.raw_clock_info_mult = ci.mult;
  g_raw_ci_pub.raw_clock_info_shift = ci.shift;
  g_raw_ci_pub.raw_clock_info_mask = ci.mask;
  g_raw_ci_pub_valid = 1;
  g_raw_ci_pub_mono_ns = now;
}

// Live HCA free-running counter, in raw device cycles (not nanoseconds): the free-running register
// itself, not the periodically-refreshed clock_info page, so it advances every call the way an
// in-flight CQE timestamp is captured. Returns 0 when the raw device is not configured.
static uint64_t raw_live_cycles(void) {
  ...
  return ns(&v.raw_clock);  // driver packs the raw cycle count into this field, not real time
}

// Live raw counter converted to real nanoseconds through the same conversion remap_raw applies to a
// CQE timestamp, so the daemon's fit is built from one definition of "raw, in ns", not two. Returns
// 0 before the first clock_info refresh.
static uint64_t raw_now_ns(void) {
  if (!g_raw_ci_pub_valid) return 0;
  const uint64_t cycles = raw_live_cycles();
  if (cycles == 0) return 0;
  return datacrumbs_timesync_raw_ts_to_ns(&g_raw_ci_pub, cycles);
}

// Sliding window of (raw, synced_phc - raw) samples; a least-squares line gives the fit the reader
// extrapolates from, so its skew term absorbs the small servo drift between the HCA clock and the
// synced PHC.
#define RAWWIN 16
...
// Read the HCA raw clock (converted to ns) bracketed around a synced-PHC read, so the (phc - raw)
// sample is free of the read gap: raw and phc share the NIC oscillator, so the bracket midpoint
// pins them to one instant. Returns 0 (no fit) when the raw device is not configured or not yet
// converted.
static uint64_t raw_paired(int synced_phc, uint64_t* phc_out) {
  uint64_t a = raw_now_ns();
  if (a == 0) return 0;
  int64_t br = phc_mono_offset(synced_phc, phc_out);
  uint64_t b = raw_now_ns();
  (void)br;
  return (a + b) / 2;
}
...
static void publish_clock_info_snapshot(struct dc_timesync_snapshot* v) {
  if (!g_raw_ci_pub_valid) return;
  v->raw_clock_info_valid = 1;
  v->raw_clock_info_updated_mono_ns = g_raw_ci_pub_mono_ns;
  v->raw_clock_info_nsec = g_raw_ci_pub.raw_clock_info_nsec;
  v->raw_clock_info_last_cycles = g_raw_ci_pub.raw_clock_info_last_cycles;
  v->raw_clock_info_frac = g_raw_ci_pub.raw_clock_info_frac;
  v->raw_clock_info_mult = g_raw_ci_pub.raw_clock_info_mult;
  v->raw_clock_info_shift = g_raw_ci_pub.raw_clock_info_shift;
  v->raw_clock_info_mask = g_raw_ci_pub.raw_clock_info_mask;
}
...
static void publish(struct dc_timesync_snapshot* s, const struct dc_timesync_snapshot* v) {
  ...
  for (uint32_t i = 0; i < v->n_clocks && i < DC_TIMESYNC_MAX_CLOCKS; i++)
    s->clocks[i] = v->clocks[i];
  s->raw_valid = v->raw_valid;
  s->raw_anchor_ns = v->raw_anchor_ns;
  s->raw_to_synced_ns = v->raw_to_synced_ns;
  s->raw_skew_ppb = v->raw_skew_ppb;
  s->raw_clock_info_valid = v->raw_clock_info_valid;
  s->raw_clock_info_updated_mono_ns = v->raw_clock_info_updated_mono_ns;
  s->raw_clock_info_nsec = v->raw_clock_info_nsec;
  s->raw_clock_info_last_cycles = v->raw_clock_info_last_cycles;
  s->raw_clock_info_frac = v->raw_clock_info_frac;
  s->raw_clock_info_mult = v->raw_clock_info_mult;
  s->raw_clock_info_shift = v->raw_clock_info_shift;
  s->raw_clock_info_mask = v->raw_clock_info_mask;
  __atomic_thread_fence(__ATOMIC_RELEASE);
...
int main(int argc, char** argv) {
  ...
  if (strcmp(role, "dump") == 0) {  // inspect the published snapshot (no NIC/sudo needed)
    ...
    printf(
        "  raw clock_info: valid=%u age=%lldms nsec=%llu last_cycles=%llu mult=%u shift=%u "
        "mask=%#llx\n",
        shm->raw_clock_info_valid,
        (long long)((mono_ns() - shm->raw_clock_info_updated_mono_ns) / 1000000),
        (unsigned long long)shm->raw_clock_info_nsec,
        (unsigned long long)shm->raw_clock_info_last_cycles, shm->raw_clock_info_mult,
        shm->raw_clock_info_shift, (unsigned long long)shm->raw_clock_info_mask);
    return 0;
  }
  ...
  if (strcmp(role, "daemon") == 0) {
    ...
    printf("# daemon self=%u ref=%u cadence=%dms snapshot=%s\n", self_id, ref_id, cadence_ms,
           snap_path);
    const char* raw_dev =
        getenv(DATACRUMBS_ENV_TIMESYNC_RAW_DEV);  // fabric ib device -> publish raw-clock fit
    if (raw_dev && *raw_dev) raw_open(raw_dev);
    const char* ci_refresh = getenv(DATACRUMBS_ENV_TIMESYNC_CLOCK_INFO_REFRESH_MS);
    if (ci_refresh && *ci_refresh) g_raw_ci_refresh_ms = atoi(ci_refresh);
    if (g_raw_ci_refresh_ms <= 0) g_raw_ci_refresh_ms = 60000;
    ...
    if (self_id == ref_id) {
      ...
      for (;;) {
        ...
        maybe_refresh_clock_info(now);
        uint64_t raw_phc = 0;
        uint64_t raw = raw_paired(phc, &raw_phc);
        raw_fit_update(raw, raw_phc, &v);
        publish_clock_info_snapshot(&v);
        publish(shm, &v);
        last_pub = now;
      }
    }
    ...
    for (;;) {
      ...
      if (do_exchange(fd, &peer, seq++, &off, &dly, &ta_s) == 0 && !warm) {
        ...
        if (wn >= 2) {
          ...
          fill_clock_registry(&v, phc);
          maybe_refresh_clock_info(mono_ns());
          uint64_t raw_phc = 0;
          uint64_t raw = raw_paired(phc, &raw_phc);
          raw_fit_update(raw, raw_phc, &v);
          publish_clock_info_snapshot(&v);
